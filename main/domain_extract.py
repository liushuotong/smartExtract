...
def extract_based_on_domain(results):
    ...
    for entry in results:
        ...
        features = entry.get('FEATURES', [])
        for feature in features:
            # 获取 'USER_PROTEIN_ID' 对应的值，并转换为字符串
            transcript_ID = str(entry.get('USER_PROTEIN_ID', ''))

            # 根据空格分割 transcript_ID，并去除多余的空格
            transcript_ID_list = transcript_ID.split()
            domain = feature.get('DOMAIN', '')
